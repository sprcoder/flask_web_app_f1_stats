...
def check_duplicate(e):
    ...
    import re
    r = re.match(".*IS601_sr2484_Users.(\w+)", e.args[0].args[1])
    if r:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            result = DB.insertOne("INSERT INTO IS601_sr2484_Users (email, username, password) VALUES (%s, %s, %s)", email, username, hash)
            if result.status:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if is_valid:
            try:
                result = DB.selectOne("SELECT id, email, username, password FROM IS601_sr2484_Users where email= %(email)s or username=%(email)s", {"email":email})
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if current_password and password and confirm:
            try:
                result = DB.selectOne("SELECT password FROM IS601_sr2484_Users where id = %s", user_id)
                if result.status and result.row:
                    ...
                    if bcrypt.check_password_hash(result.row["password"], current_password):
                        ...
                        hash = bcrypt.generate_password_hash(password)
                        try:
                            result = DB.update("UPDATE IS601_sr2484_Users SET password = %s WHERE id = %s", hash, user_id)
                            if result.status:
                ...
                flash(se, "danger")
        elif current_password or password or confirm:
            flash("Password update is not saved. Please provide all details to update password", "warning")
        
        if is_valid:
            try: # update email, username (this will trigger if nothing changed but it's fine)
                result = DB.update("UPDATE IS601_sr2484_Users SET email = %s, username = %s WHERE id = %s", email, username, user_id)
                if result.status:
    ...
    try:
        ...
        result = DB.selectOne("SELECT id, email, username FROM IS601_sr2484_Users where id = %s", user_id)
        if result.status and result.row:
            user = User(**result.row)
