...
def add():
    ...
    if form.validate_on_submit():
        try:
            result = DB.insertOne("INSERT INTO IS601_sr2484_Roles (name, description, is_active) VALUES (%s, %s, %s)",
...
def edit():
    ...
    if form.validate_on_submit() and id:
        try:
            result = DB.insertOne("UPDATE IS601_sr2484_Roles set name = %s, description = %s, is_active = %s WHERE id = %s",
    ...
    try:
        result = DB.selectOne("SELECT name, description, is_active from IS601_sr2484_Roles WHERE id = %s", id)
...
def list():
    rows = [] 
    query = """SELECT r.id as id, r.name as name, r.description as description, r.is_active as is_active, IFNULL(ur.dcount,0) as assigned  
                FROM IS601_sr2484_Roles as r LEFT JOIN (select role_id, count(*) as dcount from IS601_sr2484_UserRoles group by role_id) as ur on r.id=ur.role_id"""
    try:
...
def delete():
    ...
    if id:
        try:
            result = DB.delete("DELETE FROM IS601_sr2484_Roles WHERE id = %s", id)
            if result.status:
                flash("Deleted role", "success")
        except Exception as e:
            print(e)
            # TODO make this user-friendly
            flash("Roles have users assigned to them, only after removing all users, a role can be deleted", "danger")
    ...
    if email:
        try:
            ...
            SELECT id, email, 
                ...
                IS601_sr2484_UserRoles ur JOIN IS601_sr2484_Roles on ur.role_id = IS601_sr2484_Roles.id WHERE ur.user_id = IS601_sr2484_Users.id) as roles
            FROM IS601_sr2484_Users where email like %s limit 10
            ...
            flash(str(e), "danger")
    result = DB.selectAll("SELECT id, name FROM IS601_sr2484_Roles WHERE is_active = 1",)
    if result.status and result.rows:
        roles = result.rows
...
def apply():
    ...
    if users and roles: # we need both for this to work
        ...
        if len(mappings) > 0:
            try:
                result = DB.insertMany("INSERT INTO IS601_sr2484_UserRoles (user_id, role_id, is_active) VALUES(%s, %s, 1) ON DUPLICATE KEY UPDATE is_active = !is_active", mappings)
                if result.status:
