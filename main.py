...
def create_app(config_filename=''):
    ...
    app.register_error_handler(401, unautherized)
    cache.init_app(app)
    app.cache = cache
    app.secret_key = os.environ.get("SECRET_KEY", "missing_secret")
    app.api_host = os.environ.get("F1_HOST")
    app.api_key = os.environ.get("F1_KEY")
    ...
    with app.app_context():
        ...
        def load_user(user_id):
            ...
            try:
                result = DB.selectOne("SELECT id, email FROM IS601_sr2484_Users WHERE id = %s", user_id)
                if result.status:
